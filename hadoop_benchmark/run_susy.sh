...
# ---Find the Hadoop Streaming JAR automatically ---
...
# --- CONFIGURATION FOR SUSY INPUT ---
DATASET_FILENAME="SUSY.csv"
LOCAL_DATA_PATH="/code/$DATASET_FILENAME"
HDFS_INPUT="/input/$DATASET_FILENAME"
HDFS_OUTPUT="/output_susy_benchmark"

# Files to ship to the cluster
MAPPER_SCRIPT="mapper_susy.py"
...
echo "Uploading '$LOCAL_DATA_PATH' to HDFS..."
...
echo "--- Running MapReduce Job (SUSY Benchmark) ---"
# CRITICAL: Capture start time in milliseconds for precision
start_time=$(date +%s.%N)

# Note the change in -files and -mapper to use mapper_susy.py
